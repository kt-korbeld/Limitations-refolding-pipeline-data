...
def rm_files(loc, filetypes=['txt', 'json', 'png', 'a3m']):
    files = [i for i in os.listdir(loc) if i.split('.')[-1] in filetypes]
    for file in files:
        filepath = os.path.join(loc, file)
        os.remove(filepath)
    return
    ...
    with torch.no_grad():
        for sample_id in tqdm(sample_ids, desc="Processing samples"):
            ...
            for pctg in scrambled_pctgs:
                ...
                colab_dir = os.path.join(pctg_dir, 'colab')
                os.makedirs(colab_dir, exist_ok=True)
                all_colab_pdbs = []
                #all_esm_pdbs = []
                for batch in tqdm(batches, desc=f"Running ColabFold for {pctg} for {sample_id}"):
                    colab_args = ["colabfold_batch",
                                  "--num-models", "1",
                                  "--num-recycle", "3",
                                  "--msa-mode", "single_sequence",
                                  scrambled_fasta_path, colab_dir]
                    try:
                        ...
                        _ = rm_files(colab_dir)
                    except Exception as e:
                        log.error(f"ColabFold failed for batch: {e}")
                        continue
                ...
                for i, pdb in enumerate(pdbs):
                    ...
                    #colab_sample_path = os.path.join(esmf_dir, f'esmf_{sample_id}.pdb')
                    ...
                    # rename pdb files
                    if i == 0:
                        colab_sample_path = os.path.join(colab_dir, 'colab_{}.pdb'.format(sample_id))
                    ...
                    #colab_feats = du.parse_pdb_feats('folded_sample', colab_sample_path, calc_dssp=calc_non_coil_rmsd)
                    colab_feats = du.parse_pdb_feats('folded_sample', colab_sample_path)
                    assert sample_feats['bb_positions'].shape[0] == colab_feats['bb_positions'].shape[0], 'Number of residues do not match'
                    sample_seq = du.aatype_to_seq(sample_feats['aatype'])

                    try:
                        _, tm_score = metrics.calc_tm_score(
                            sample_feats['bb_positions'], colab_feats['bb_positions'],
                    ...
                    helix, strand, coil = get_sse(colab_sample_path)
